...
void main() {
  late MockNowPlayingMoviesBloc mockNotifier;
  late MockMovieDetailBloc mockMovieDetailNotifier;
  late MockNavigatorObserver mockNavigatorObserver;

  setUp(() {
    mockNotifier = MockNowPlayingMoviesBloc();
  ...
  testWidgets('Page should display center progress bar when loading',
    ...
    when(mockNotifier.state).thenReturn(NowPlayingMoviesLoading());
  ...
  testWidgets('Page should display ListView when data is loaded',
      (WidgetTester tester) async {
    when(mockNotifier.stream).thenAnswer((_) => (Stream.fromIterable([])));
    when(mockNotifier.state).thenReturn(const NowPlayingMoviesSuccess(<Movie>[]));
  ...
  testWidgets('should return Movie Card when Request State is Loaded',
      (tester) async {
    when(mockNotifier.stream).thenAnswer((_) => (Stream.fromIterable([])));
    when(mockNotifier.state).thenReturn(NowPlayingMoviesSuccess([testMovie]));
